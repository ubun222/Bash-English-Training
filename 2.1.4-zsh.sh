...
#theam="$inline"
la=$((${#inline}*2-2))
for STN in $(seq ${#inline});do
if [[  "${inline:$STN:1}" == [\ -\~]  ]] ;then
la=$((la-1))
fi
done
down1=$((la/$((COLUMN+1))+1))
...
ishprt "$question\033[2m\033[3m ‹———› \033[0m$bots\r"
[[  $COLUMN -lt $length  ]] && ishprt "\033[$(($((length-1))/COLUMN))A"
ishprt "\033[1m$question\033[0m\033[2m\033[3m ‹———› \033[0m"
continue
elif [[  "$premode" -eq 2  ]];then
...
[[  "$once" -eq 0  ]] && once=1 && printf "\033[1m" && ishprt "$(modify "$am1")" && eval ishprt "\$down_1" && ishprt "$enter\033[0m\033[1m\033[36m ›\033[0m$enter" && continue
printf "$enter"
break
fi
...
[[  "$once" -eq 0  ]] && once=1 && [[  $sub -eq 0  ]] && printf "\033[1m" && ishprt "$(modify "$am1")" && eval ishprt "\$down_1" && ishprt "$enter\033[0m\033[1m\033[36m ›\033[0m$enter" && continue
if [[  $sub -eq 0  ]];then 
getin=0
order=$ascanf
break
...
printf  "\033[0m$question1\\033[2m\033[3m ‹———› \033[0m$bot"
[[  $COLUMN -lt $length  ]] && printf "\r\033[$(($((length-1))/COLUMN))A"
printf "\r$(modify "$question1")\033[0m\033[2m\033[3m ‹———› \033[0m"
Readen
...
helptxt="$0 [-p [-rR] -a -t txt -i -s -j] *.txt
-p 通关模式(循环答题直到全部做对)
-r 错题集模式(在txt/CORRECT文件夹内自动收集错题集)
-R 剔除模式(直接对当前加载的词表文件进行读写)
-a 答题辅助(自动补全adj. v. n.等词性信息)
-t txt文件夹路径
-s 验证词表格式
